...
public record ModpackInfo(ModpackFormat format, String name, String version, String mcVersion,
    ...
    /** The value this pack needs in the launcher's LOADER dropdown ("Vanilla"/"Fabric"/"Forge"), or
     *  null when it needs a loader DeyLauncher doesn't install yet (NeoForge / Quilt). */
    public String launcherLoader() {
        ...
        return null; // NeoForge / Quilt / anything else -- we refuse the install instead of guessing
    }
    ...
    public String blockingProblem() {
        if (!supportedLoader()) {
            return "This pack needs " + loaderName + ", which DeyLauncher doesn't install yet -- "
                    + "only Vanilla, Fabric and Forge are supported.";
        }
        return null;
    }
